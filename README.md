...
Документ обновлен после локальной проверки проекта и текущего пакета исправлений 01.07.2026.
...
| Проверка | Результат |
|---|---:|
| `git status --short` перед коммитом | изменения только в `README.md`, `charts.tsx`, `components/FoodStreakCard.tsx` |
...
| `npm run test:unit` | пройдено, 48 файлов / 187 тестов |
...
- Стабилизирована мобильная разметка карточки `Food Streak`: длинный бейдж серии больше не должен расширять экран.
- Для обзорных Recharts-графиков отключены входные анимации, чтобы прокрутка на мобильных была ровнее.
- Карточки обзорных графиков получили `min-w-0`, чтобы содержимое не создавало горизонтальное переполнение.
- README синхронизирован с текущими проверками: 20 миграций, 30 таблиц, 48 unit-файлов и 187 тестов.
