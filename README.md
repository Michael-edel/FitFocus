...
Документ обновлен после локальной проверки проекта и текущего пакета исправлений 29.08.2026.
...
| Проверка | Результат |
|---|---:|
| Текущий пакет изменений | Атомарная синхронизация profile/state, защита от stale overwrite, очистка локальных данных после удаления аккаунта, runtime-проверка cloud profile и regression-тесты |
...
| `npm run test:unit` | пройдено, 61 файл / 249 тестов |
| `npm run test:e2e -- e2e/pwa-shell.spec.ts e2e/onboarding.spec.ts e2e/push-settings.spec.ts` | не выполнено: все 21 сценарий остановлены до запуска из-за отсутствующих бинарников Playwright Chromium/Firefox/WebKit |
...
- `/api/profile` записывает профиль через условную проверку версии (CAS): существующий профиль нельзя перезаписать с `baseVersion=0`, а два параллельных клиента с одной версией принимает только один.
- `/api/profile` больше не смешивает запись профиля и KV-состояния: legacy-поле `stateItems` отклоняется до записи с указанием использовать `/api/state`, поэтому KV-конфликт не оставляет частично применённый профиль.
- `/api/state` записывает каждый ключ через условный `INSERT` или `UPDATE`, проверяет версии непосредственно в записи и отклоняет дубликаты ключей в одном запросе.
- `/api/bootstrap`, `/api/export` и `/api/mobile/token` проверяют beta-доступ после авторизации и не отдают данные или мобильный токен сессии без активированного приглашения.
- Ответ AI на сервере принимается как `unknown` и используется только после проверки JSON-объекта; feature flags используют типизированный D1-результат.
- `sw.ts` содержит рабочий маркер `self.__WB_MANIFEST`; production-сборка внедряет 52 записи precache.
- `profileSync.ts` больше не выполняет полную автоматическую перезапись локальным устаревшим профилем после конфликта; изменения профиля и state-ключи синхронизируются отдельными проверяемыми запросами.
- Ответ профиля при загрузке из облака проходит runtime-проверку перед передачей в состояние приложения.
- Удаление аккаунта после успешного ответа сервера удаляет пользовательские данные из `localStorage` и не ставит эти локальные удаления в облачную очередь; данные другого аккаунта на том же устройстве не затрагиваются.
- Добавлены regression-тесты для проверки profile CAS, удаления аккаунта и очистки локальной приватности; существующие проверки `/api/state` обновлены под метаданные условной записи D1.
- В текущем окружении production E2E не выполнены: Playwright установлен без соответствующих браузерных бинарников. Unit-тесты, typecheck, API/auth/schema/privacy/admin-инварианты и production build пройдены.
- README синхронизирован с текущими проверками: 20 миграций, 30 таблиц, 61 unit-файл и 249 тестов.
...
| `npm run check:schema` | пройдено, 20 миграций / 30 таблиц |
| `npm run check:privacy` | пройдено, 27 пользовательских таблиц |
| `npm run check:admin-guards` | пройдено, 15 файлов |
| `npm run test:unit` | пройдено, 61 файл / 249 тестов |
| `npm run typecheck` | пройдено |
| `npm run build` | пройдено |
| `npm run test:e2e -- e2e/pwa-shell.spec.ts e2e/onboarding.spec.ts e2e/push-settings.spec.ts` | не выполнено: отсутствуют браузеры Playwright; 21 сценарий остановлен до запуска |
...
- AI telemetry больше не сохраняет raw текст исключения при сбое запроса к провайдеру; в `ai_events` пишется стабильный код `AI_FETCH_*`.
...
- Верхний слой `geminiService.ts` типизирует env, AI proxy response, AI text extraction и нормализацию списка покупок без прежних `as any`; кириллические единицы `г/кг` в списке покупок распознаются через явную границу, а не через `\b`.
...
- `PlanScreen.tsx` больше не использует `any`: props, AI-план, недельное меню, семейное меню, список покупок и пользовательские профили типизированы через существующие модели проекта.
...
- README синхронизирован с проверками на момент пакета: 20 миграций, 30 таблиц, 54 unit-файла и 220 тестов.
...
- Основная модель: `gpt-5.6-luna` через OpenAI Responses API.
- OpenAI преобразует текстовые и графические входы в формат Responses API; структурированные ответы передаются через `text.format.json_schema`.
- Старые модели `gemini-2.5-flash`, `gemini-2.5-pro` и `gemini-2.5-flash-lite` временно разрешены только для совместимости со старыми клиентами.
- При отсутствии `OPENAI_API_KEY` запросы Luna не выполняются; ключ хранится только в Cloudflare secret.
...
- `GEMINI_TIMEOUT_MS`
- `GEMINI_API_KEY` только для совместимости со старыми клиентами
- `FREE_AI_DAILY_LIMIT`
- `PRO_AI_DAILY_LIMIT`
