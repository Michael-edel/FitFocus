...
const currentBuildReleaseNote: ReleaseNote = {
  ...
  summary: BUILD_SOURCE.branch === 'main'
    ? 'Сборка main обновляется автоматически при каждом push. Релизная версия продукта меняется отдельно, а ниже показаны понятный номер текущей сборки, SHA, последние изменения и схема версионирования.'
  ...
  groups: [
    {
      ...
      items: [
        ...
        `Сборка main: ${BUILD_SOURCE.shortSha} — это идентификатор текущего push/деплоя; он меняется автоматически при каждом изменении в main.`,
