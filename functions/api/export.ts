...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const aiRateLimits = await allRows(db, "SELECT kind, bucket_key, count, reset_at, updated_at FROM ai_rate_limits WHERE user_id = ? ORDER BY updated_at DESC", userId);
  ...
  const achievements = await allRows(db, "SELECT achievement_key, unlocked_at, meta_json FROM user_achievements WHERE user_id = ? ORDER BY unlocked_at DESC", userId);
  ...
  const familyInvitesCreated = await allRows(db, "SELECT code, family_id, expires_at, max_uses, created_by_user_id, created_at, used_by_user_id, used_at FROM family_invites WHERE created_by_user_id = ? ORDER BY created_at DESC", userId);
  const familyInvitesUsed = await allRows(db, "SELECT code, family_id, expires_at, max_uses, created_by_user_id, created_at, used_by_user_id, used_at FROM family_invites WHERE used_by_user_id = ? ORDER BY used_at DESC", userId);
  const recipes = await allRows(db, "SELECT id, title, ingredients_json, steps_json, nutrition_json, tags_json, source, created_at, updated_at FROM recipes WHERE user_id = ? ORDER BY created_at DESC", userId);
