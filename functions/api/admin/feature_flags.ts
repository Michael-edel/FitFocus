// Cloudflare Pages Function: /api/admin/feature_flags
...
export const onRequestPut: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const rollout = Number.isFinite(body?.rollout_percentage) ? Math.max(0, Math.min(100, Number(body.rollout_percentage))) : 100;
